 ...
 const { bookingId } = await context.params
  ...
  // Verify the Google Doc is publicly accessible
    try {
      ...
      if (isRestricted) {
        return NextResponse.json(
{ error: 'This Google Doc isn\'t publicly shared. Please open your doc, click Share, change "General access" to "Anyone with the link can comment / edit", then try again.' }          { status: 422 }
        )
      }
    } catch {
