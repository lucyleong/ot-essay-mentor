...
export function surveyEmail(d: SurveyEmailData) {
  ...
  const content = `
    <h1 style="margin:0 0 6px;font-size:20px;font-weight:500;color:#2C2C2A;">
      ${isStudent ? 'How did your session go?' : 'Session report'}
    </h1>

    <p style="margin:0 0 12px;font-size:14px;color:#5F5E5A;line-height:1.5;">
      Hi ${d.recipientName},
      ${isStudent
    ...
    <p style="margin:0 0 12px;font-size:14px;color:#2C2C2A;line-height:1.5;">
      ${isStudent
    ...
    <a href="${d.surveyUrl}" style="display:inline-block;background:#534AB7;color:#EEEDFE;text-decoration:none;font-size:14px;font-weight:500;padding:10px 24px;border-radius:8px;">
    ...
    <p style="margin:0 0 10px;font-size:14px;color:#2C2C2A;line-height:1.5;">
      Need more help with your essay? Book another appointment anytime.
    </p>

    <a href="${process.env.NEXT_PUBLIC_APP_URL}/book?code=${process.env.NEXT_PUBLIC_BOOKING_CODE}" style="display:inline-block;background:#534AB7;color:#EEEDFE;text-decoration:none;font-size:14px;font-weight:500;padding:10px 24px;border-radius:8px;">
      Book another appointment →
    </a>
    ` : `
    <p style="margin:16px 0 0;font-size:11px;color:#888780;">
